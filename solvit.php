							...
							<table class="table forum table-striped">
								...
								<tbody>
									<tr>
										...
										<td class="name">
										  <h4><a href="solve.php?varname=
										  <?php $var_value = "Hardware"; echo $var_value?>&type=<?php $type = "Computer"; echo $type ?>">Hardware</a><br><small>Some description</small></h4>
										</td>
										<td class="text-center hidden-xs hidden-sm"><a href="#"></a></td>
										<td class="text-center hidden-xs hidden-sm"><a href="#"></a></td>
										<td class="hidden-xs hidden-sm">by <a href="#">
											<?php $query = "SELECT datePublished,userId FROM posts WHERE problemType like '$hardware' and itemType like '$computer'";
												$data = $MySQLi_CON->query($query);
												$items = array();
												foreach($data as $key) {
													$items[] = $key['datePublished'];
												}		
												$date1 = max($items); 
											
												$userQuery = "select userId from posts where datePublished like '$date1'";
												$userData = $MySQLi_CON->query($userQuery);
												foreach($userData as $userKey) {
													$userD = $userKey['userId'];
												}	
												
												$usersQuery= "select userName from users where userId = $userD";
												$usersData = $MySQLi_CON->query($usersQuery);
												foreach($usersData as $usersKey) {
													$user1 = $usersKey['userName'];
												}	
												echo $user1;
											?>
											</a>
											...
											<?php 
												$query = "SELECT datePublished,userId FROM posts WHERE problemType like '$hardware' and itemType like '$computer'";
												$data = $MySQLi_CON->query($query);
												$items = array();
												foreach($data as $key) {
													$items[] = $key['datePublished'];
												}		
												$date1 = max($items); echo $date1 ?>
											</small>
										</td>
									</tr>
									<tr>
										...
										<td class="text-center hidden-xs hidden-sm"><a href="#"></a></td>
										<td class="text-center hidden-xs hidden-sm"><a href="#"></a></td>
										<td class="hidden-xs hidden-sm">by <a href="#"><?php $query = "SELECT datePublished,userId FROM posts WHERE problemType like '$software' and itemType like '$computer'";
											$data = $MySQLi_CON->query($query);
											$items = array();
											foreach($data as $key) {
												$items[] = $key['datePublished'];
											...
											echo $user1;
											?></a><br><small><i class="fa fa-clock-o"></i><?php $query1 = "SELECT datePublished,userId FROM posts WHERE problemType like '$software' and itemType like '$computer'";
											$data1 = $MySQLi_CON->query($query1);
											$items1 = array();
											foreach($data1 as $key1) {
												$items1[] = $key1['datePublished'];
											}		
											$date2 = max($items1); echo $date2 ?></small>
										</td>
									</tr>
								</tbody>
							</table>
							<table class="table forum table-striped">
								...
								<tbody>
									<tr>
										...
										<td class="text-center hidden-xs hidden-sm"><a href="#"></a></td>
										<td class="text-center hidden-xs hidden-sm"><a href="#"></a></td>
										<td class="hidden-xs hidden-sm">by <a href="#"><?php $query = "SELECT datePublised,userId FROM posts WHERE problemType like '$hardware' and itemType like '$mobile'";
											 $data = $MySQLi_CON->query($query);
											 $items = array();
												foreach($data as $key) {
												 $items[] = $key['datePublished'];
												...
												echo $user1;
												?></a><br><small><i class="fa fa-clock-o"></i>
												<?php 
													$query2 = "SELECT datePublished,userId FROM posts WHERE problemType like '$hardware' and itemType like '$mobile'";
													$data2 = $MySQLi_CON->query($query2);
													$items2 = array();
													foreach($data2 as $key2) {
														$items2[] = $key2['datePublished'];
													}		
													$date3 = max($items2); echo $date3 ?></small>
										</td>
									</tr>
									<tr>
										<td class="text-center"><i class="fa fa-exclamation fa-2x text-danger"></i></td>
										<td>
											<h4><a href="solve.php?varname=
											<?php 
												$var_value = "Software"; echo $var_value 
											?>
											&type=
											<?php 
												$type = "Mobile"; echo $type 
											?>
											">Software</a><br><small>Some description</small></h4>
										</td>
										<td class="text-center hidden-xs hidden-sm"><a href="#"></a></td>
										<td class="text-center hidden-xs hidden-sm"><a href="#"></a></td>
										<td class="hidden-xs hidden-sm">by <a href="#">
										<?php 
											$query = "SELECT dateCreated,userId FROM posts WHERE problemType like '$software' and itemType like '$mobile'";
											$data = $MySQLi_CON->query($query);
											$items = array();
											foreach($data as $key) {
												$items[] = $key['date'];
