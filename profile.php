...
include("head.php");
	...
	foreach( $result3 as $row ) {
	  ...
	  if(isset($_POST['submit'])) {
		...
		$sql = "UPDATE user SET user_Name=:name, birthdate=:dob, mobile=:phone, occupation=:expert, experience=:exp, address=:address WHERE user_ID=:id";
 ...
 if(isset($_POST['submit2'])) {
		...
		if($pass == $pass1){
		$hash = password_hash($pass, PASSWORD_DEFAULT);	
		$sql = "UPDATE user SET email=:email, password=:hash, chat=:chat, paypal=:paypal WHERE user_ID=:id";
		$stmt = $MySQLi_CON->prepare($sql);
		$stmt->bindValue(":email", $email, PDO::PARAM_STR);
                      ...
                      <form action="#" method="post">
                        ...
                        <div class="form-group has-feedback">
                          ...
                          <span class="glyphicon glyphicon-lock form-control-feedback"></span>
						   ...
						   <div class="form-group has-feedback">
                          <input type="text" name="chat" class="form-control" value='<?php echo $chat1; ?>'  placeholder="Chat ID">
                          <span class="glyphicon glyphicon-lock form-control-feedback"></span>
                        </div>
						 <div class="form-group has-feedback">
